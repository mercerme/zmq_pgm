...
void EPGMPublisherThread() {
	...
	zmq_bind(publisher, "epgm://239.192.1.23:5555");                //pub端使用bind连接组播地址 
	...
	*/
	...
	while (index<1000000) {                      
		...
		head->sequence = index;
		zmq_send(publisher, head, sizeof(DAMarketdataHead), 0);                          //ZMQ的send函数后面四个参数依次为，socket，buffer，buffer大小，最后一个设置为0即可
		index++;
